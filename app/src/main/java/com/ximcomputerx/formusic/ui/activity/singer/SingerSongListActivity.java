package com.ximcomputerx.formusic.ui.activity.singer;
 ...
 * @AUTHOR HACKER
 */
public class SingerSongListActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        singerInfo = (SingerInfo) intent.getSerializableExtra(SONGINFO);

        String singerId = SharedPreferencesUtil.getStringPreferences(Constant.PREFERENCES, Constant.PLAY_LIST_SINGER, "");
        if (singerId.equals(singerInfo.getId())) {
            addFlag = true;
        }
    ...
    private void initHeadData() {
        TextViewBinder.setTextView(tv_name, singerInfo.getName());
        TextViewBinder.setTextView(tv_des, singerInfo.getName());
    }
    ...
    private void setPicture() {
        //GlideImageLoaderUtil.displayImage(singerInfo.getPicUrl(), iv_one_photo);
        GlideImageLoaderUtil.displayRoundImage(singerInfo.getPicUrl(), iv_one_photo, R.mipmap.default_cover);
        // "14":模糊度；"3":图片缩放3倍后再进行模糊
        Glide.with(this).load(singerInfo.getPicUrl())
                .dontAnimate()
    ...
    private void setTitleBar() {
        ...
        title_tool_bar.setTitle(singerInfo.getName());
...
//        DebugUtil.error("----alpha:  " + alpha);
    ...
    protected void onClick(View v) {
        switch (v.getId()) {
            case R.id.ll_play_all:
                SingerSongListActivity.this.position = 0;
                playList();
                addFlag = true;
    ...
    private void initSongListData() {
        getApiWrapper(true).singerHotList(singerInfo.getId())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<SingerHotListInfo>() {
                    @Override
                    ...
                    @Override
                    public void onNext(SingerHotListInfo singerHotListInfo) {
                        if (singerHotListInfo != null) {
                            if (singerHotListInfo.getSongs() != null) {
                                songInfos = singerHotListInfo.getSongs();
                                ...
                                listSongAdapter.setOnItemClickListener(new ListSongAdapter.OnItemClickListener() {
                                    ...
                                    public void onItemClick(View view, int position, Object data) {
                                        if (!addFlag) {
                                            SingerSongListActivity.this.position = position;
                                            playList();
                                            addFlag = true;
                                        } else {
                                            // 发送位置更新事件
    ...
    private void playList() {
        if (songInfos != null && songInfos.size() > 0) {
            ...
            //Toasty.normal(getApplicationContext(), "删除成功", Toasty.LENGTH_SHORT).show();
                ...
                .subscribe(new Subscriber<SongUrlListInfo<SongUrlInfo>>() {
                    ...
                    public void onNext(SongUrlListInfo<SongUrlInfo> songUrlListInfo) {
                        ...
                        if (songUrlListInfo != null) {
                            ...
                            clearListType();
                            // 保存歌手标记
                            SharedPreferencesUtil.setStringPreferences(Constant.PREFERENCES, Constant.PLAY_LIST_SINGER, singerInfo.getId());
                            //Toasty.normal(getApplicationContext(), "添加成功", Toasty.LENGTH_SHORT).show();
                        } else {
