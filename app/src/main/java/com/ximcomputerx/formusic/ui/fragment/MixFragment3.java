 ...
 * @AUTHOR HACKER
 */
public class MixFragment3 extends BaseFragment implements SwipeRecyclerView.LoadMoreListener {
    @Bind(R.id.rv_mix_3)
    protected SwipeRecyclerView rv_mix_3;
    ...
    protected int setLayoutId() {
        return R.layout.fragment_mix_3;
    }

    @Override
    protected void initView(View contentView) {
        GridLayoutManager layoutManager = new GridLayoutManager(getContext(), 3);
        rv_mix_3.setLayoutManager(layoutManager);
        CustomLoadMoreView customLoadMoreView = new CustomLoadMoreView(context);
        rv_mix_3.addFooterView(customLoadMoreView);
        rv_mix_3.setLoadMoreView(customLoadMoreView);
        rv_mix_3.setLoadMoreListener(this);
    }
    ...
    private void initMixData(boolean show) {
        getApiWrapper(show).mix(pageNumer, "hot", "电子", pageoffset)
                    ...
                    public void onNext(MixListInfo<Object> mixListInfo) {
                        if (mixListInfo != null && mixListInfo.getPlaylists() != null) {
                            if (pageIndex == 1) {
                                ...
                                rv_mix_3.setAdapter(listMixAdapter);
                            ...
                            if (mixListInfo.getPlaylists().size() < pageNumer) {
                                rv_mix_3.loadMoreFinish(false, false);
                            } else {
                                rv_mix_3.loadMoreFinish(false, true);
                            }
                        } else {
                            if (pageIndex == 1) {
                                rv_mix_3.setAdapter(null);
                            }
                            // 数据完更多数据，一定要调用这个方法
                            // 第一个参数：表示此次数据是否为空
                            // 第二个参数：表示是否还有更多数据
                            rv_mix_3.loadMoreFinish(true, false);
                        }
                    }
                });
    }
