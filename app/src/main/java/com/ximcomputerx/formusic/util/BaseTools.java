...
public class BaseTools {
    ...
    public static String getVersionName() {
        // 获取packagemanager的实例
        PackageManager packageManager = ForMusicApplication.getInstance().getPackageManager();
        // getPackageName()是你当前类的包名，0代表是获取版本信息
        PackageInfo packInfo = null;
        try {
            packInfo = packageManager.getPackageInfo(ForMusicApplication.getInstance().getPackageName(), 0);
    ...
    public static void copy(String content) {
        if (!TextUtils.isEmpty(content)) {
            // 得到剪贴板管理器
            ClipboardManager cmb = (ClipboardManager) ForMusicApplication.getInstance().getSystemService(Context.CLIPBOARD_SERVICE);
     ...
     */
    public static boolean isForMusicApplicationlicationAvilible(Context context, String appPackageName) {
        try {
            // 获取packagemanager
