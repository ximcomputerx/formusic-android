...
public class CoverLoader {
    ...
    public Bitmap loadThumb(MusicInfo music) {
        return loadCover(music, Type.THUMB);
    }

    public Bitmap loadRound(MusicInfo music) {
        return loadCover(music, Type.ROUND);
    }

    public Bitmap loadBlur(MusicInfo music) {
        return loadCover(music, Type.BLUR);
    }

    private Bitmap loadCover(MusicInfo music, Type type) {
        Bitmap bitmap;
    ...
    private String getKey(MusicInfo music) {
        ...
        if (music.getType() == MusicInfo.Type.LOCAL && music.getAlbumId() > 0) {
            return String.valueOf(music.getAlbumId());
        } else if (music.getType() == MusicInfo.Type.ONLINE && !TextUtils.isEmpty(music.getCoverPath())) {
    ...
    private Bitmap loadCoverByType(MusicInfo music, Type type) {
        Bitmap bitmap;
